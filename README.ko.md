...
| Connector | 몸 없는 커넥터 — 동사가 외부 REST API 를 부르는 패키지. `api` 서비스가 REST 베이스와 `auth` 형태를 선언하고, 값은 vault 의 `<패키지>/<서비스>` 에 앉는다. 기판이 호출마다 붙이므로 동사는 자격을 손에 쥐지 않고, 선언한 base 밖으로도 나가지 못한다. |
