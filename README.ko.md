...
| Agents | 페르소나(`AGENT.md`)와 스킬, 슬래시 커맨드, 서브에이전트 dispatch, 그리고 선택 `greeting` — 빈 대화의 첫 줄로, 창구가 아니라 말하는 쪽 소속이다. 하네스에는 중립 번들로 전달되고, 네이티브 형식으로의 번역은 전부 어댑터 소유. `default: true` 가 착지 에이전트 — 에이전트를 선언한 패키지는 착지가 정해져야 하고(그 표기 또는 짧은 이름과 같은 에이전트), 아니면 설치가 거부한다. 세션은 페르소나 위에 서므로 `agents[]` 밖 이름으로는 턴이 열리지 않는다. 직접 대화는 선언하지 않는다 — 에이전트가 있고 `view` 가 없으면 `/pkg/<이름>/view/` 에 전체 화면 대화가 공짜로 선다. `dirs` 는 이 에이전트가 여는 `dir` 서비스의 이름 — workspace 밖 폴더는 세션에 경로가 아니라 도구로 닿는다. |
| Scripts | 동사. `scripts/<이름>.ts`가 `async (input, ctx) => JSON`을 기본 수출. |
| Services | 형태는 넷. `source`(자기 몸, 컨테이너 또는 프로세스), `url`(원격 MCP 접점), `api`(원격 REST 베이스), `dir`(기판이 문으로 세우는 폴더). 자격은 밖으로 나가는 둘(`url`·`api`)에만 앉는다. `dir` 은 경로가 아니라 이름으로 닿는다 — 동사는 `ctx.service(<이름>).call("list"|"read"|"write"|"remove", …)`, 세션은 `agents[].dirs` 로 캡을 선언한 폴더의 `dir__<이름>__*` 도구를 본다. 선언한 경로는 로컬 기본 바인딩이고, 설치 결재가 갈아 끼우며 조직 기판은 같은 이름을 자기 볼륨 좌표로 푼다. token 자격은 입력 칸의 형태(`auth.fields` — 채널 `credential.fields` 와 같은 칸 어휘에, `Authorization` 으로 나갈 칸 하나에만 `header: true`), 필수 여부(`auth.required`, 미선언 = true; false 면 없어도 돌고 그 기능만 꺼진다), 발급처 안내(`auth.help`)를 선언한다. 콘솔의 연결 화면(`/connect`)이 이 선언으로 전 패키지의 바깥 자격을 그리고, 동사는 비밀을 쥐지 않는다 — `ctx.service(<이름>).connected()` 로 있는지 묻고 `.fields()` 로 비밀 아닌 칸만 읽는다. |
...
6. **최소 지반.** 세션이 딛는 땅은 설치 때 결재된 workspace 하나다. 폴더가 더 필요하면 `dir` 서비스를 선언한다 — 세션은 그것을 딛는 게 아니라 도구(`dir__<이름>__*`)로 부르고, 그 폴더의 경로를 알지 못한다. 기판 홈(`~/.relay`)은 모든 세션에서 항상 막히고 `dir` 로도 열 수 없다(설치가 거부한다).
