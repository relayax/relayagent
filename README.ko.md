...
| Services | 형태는 넷. `source`(자기 몸, 컨테이너 또는 프로세스), `url`(원격 MCP 접점), `api`(원격 REST 베이스), `dir`(기판이 문으로 세우는 폴더). 자격은 밖으로 나가는 둘(`url`·`api`)에만 앉는다. `dir` 은 경로가 아니라 이름으로 닿는다 — 동사는 `ctx.service(<이름>).call("list"|"read"|"write"|"remove", …)` 로 쓰고, 세션은 폴더를 직접 만지지 않는다 — 그 폴더를 감싼 동사로만 닿는다. 선언한 경로는 로컬 기본 바인딩이고, 설치 결재가 갈아 끼우며 조직 기판은 같은 이름을 자기 볼륨 좌표로 푼다. token 자격은 입력 칸의 형태(`auth.fields` — 채널 `credential.fields` 와 같은 칸 어휘에, `Authorization` 으로 나갈 칸 하나에만 `header: true`), 필수 여부(`auth.required`, 미선언 = true; false 면 없어도 돌고 그 기능만 꺼진다), 발급처 안내(`auth.help`)를 선언한다. 콘솔의 연결 화면(`/connect`)이 이 선언으로 전 패키지의 바깥 자격을 그리고, 동사는 비밀을 쥐지 않는다 — `ctx.service(<이름>).connected()` 로 있는지 묻고 `.fields()` 로 비밀 아닌 칸만 읽는다. |
| Connector | 몸 없는 커넥터 — 동사가 외부 REST API 를 부르는 패키지. `api` 서비스가 REST 베이스와 `auth` 형태를 선언하고, 값은 vault 의 `<패키지>/<서비스>` 에 앉는다. 기판이 호출마다 붙이므로 동사는 자격을 손에 쥐지 않고, 선언한 base 밖으로도 나가지 못한다. `auth.scheme` 은 `Authorization` 접두를 정한다(예: Unsplash 의 `Client-ID`) — 미선언 = `Bearer`. |
