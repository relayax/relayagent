...
| Services | 형태는 넷. `source`(자기 몸, 컨테이너 또는 프로세스), `url`(원격 MCP 접점), `api`(원격 REST 베이스), `dir`(기판이 문으로 세우는 폴더). 자격은 밖으로 나가는 둘(`url`·`api`)에만 앉는다. `dir` 은 경로가 아니라 이름으로 닿는다 — 동사는 `ctx.service(<이름>).call("list"|"read"|"write"|"remove", …)`, 세션은 `agents[].dirs` 로 캡을 선언한 폴더의 `dir__<이름>__*` 도구를 본다. 선언한 경로는 로컬 기본 바인딩이고, 설치 결재가 갈아 끼우며 조직 기판은 같은 이름을 자기 볼륨 좌표로 푼다. |
