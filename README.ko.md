...
| Harness | 패키지에 동봉되어 에이전트를 실행하는 어댑터. 시스템 패키지에 Claude Code, Codex, Kimi, Pi 어댑터 동봉. 동사: `session`, `setup`, `models`, `commands`, `info`(+선택 `login`, `serve` — 턴마다 프로세스를 갈지 않고 stdin 으로 턴을 주입받는 상주 세션). 계약 적합성은 `relay harness-check`가 판정하고, 계약 전문은 [docs/harness-protocol.md](docs/harness-protocol.md)에 있습니다. variant 는 `binary`(`{name, manager, package, version?}`)를 선언할 수 있고, 그러면 기판이 그 CLI 를 자기 자리에 설치해 PATH 앞에 둡니다 — 호스트의 전역 설치가 없거나 깨져 있어도 패키지가 멈추지 않습니다. |
