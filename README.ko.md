...
| Surfaces | 패키지가 사람을 만나는 면. 중심은 `view`: 패키지가 배송하는 웹 UI로, 설치가 빌드하고 데몬이 `/pkg/<이름>/view/`로 호스팅하며 패키지 토큰으로 자기 에이전트의 동사에 연결된다. `chat`(직접 대화)과 `channels`(Discord, Slack 등 어댑터)는 추가 문. `components`는 npm 패키지(소스 + `package.json`)를 수출한다 — 다른 패키지의 view가 components edge로 빌드 의존으로 소비하며, 기판이 설치 시점에 tgz로 굽는다(레지스트리 없음). |
