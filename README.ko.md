...
| Surfaces | 패키지가 사람을 만나는 면. 중심은 `view`: 패키지가 배송하는 웹 UI로, 설치가 빌드하고 데몬이 `/pkg/<이름>/view/`로 호스팅하며 패키지 토큰으로 자기 에이전트의 동사에 연결된다. `channels`(Discord, Slack 등 어댑터)는 추가 문 — 채널은 자기가 요구하는 자격의 *형태*를 선언하고(`credential.fields`), 콘솔이 그 선언으로 입력 칸을 그린다(사람이 JSON을 손으로 조립하지 않는다). `components`는 **자립 ESM 번들**을 수출한다(스타일도 그 안에 탄다) — 다른 패키지의 화면이 런타임에 마운트한다: 기판이 번들을 주소로 서빙하고 소비자 문서에 import map을 심으므로, 소비자는 `import { mount } from "<제공자 이름>"`만 쓰고 주소를 조립하지 않는다. React로 저작해도 그대로다 — React가 번들 안에 들어가므로 소비자에게는 프레임워크도 빌드도 필요 없다. |
| Harness | 패키지의 에이전트를 실행하는 어댑터. **어댑터는 공유되지 패키지 소속이 아닙니다**: 콘솔 패키지가 Claude Code, Codex, Kimi, Pi 를 동봉하고 기판이 그것을 풀로 펴서 모든 패키지가 후보로 봅니다 — `variants` 미선언 = 풀 전체. 동봉은 풀에 없는 하네스를 데려올 때 씁니다. 좁히려면 `harness.requires` 에 **하네스 이름이 아니라 능력**을 적고(그래야 잠근 이유가 선언에 남고, 다른 하네스가 그 능력을 구현하면 다시 열립니다), 저자가 만든 자리는 `harness.prefers`, 실제로 돌려본 것은 `harness.verified` 로 표시합니다. 자격은 `llm/<provider>` 에 앉아 패키지 간 공유되고, 연결 화면이 그것을 잇는 자리입니다. 동사: `session`, `setup`, `models`, `commands`, `info`(+선택 `login`, `serve` — 턴마다 프로세스를 갈지 않고 stdin 으로 턴을 주입받는 상주 세션). 계약 적합성은 `relay harness-check`가 판정하고, 계약 전문은 [docs/harness-protocol.md](docs/harness-protocol.md)에 있습니다. variant 가 모는 CLI 는 `requires.binaries` 에 선언합니다 — `manager`+`package` 레시피가 있으면 없을 때 기판이 직접 깔고(`~/.relay/bin/<패키지>/`, PATH 앞), variant 의 `binary: <name>` 은 그 항목의 참조라 깨진 호스트 설치도 setup 실패 시 기판 사본으로 대체됩니다. |
| Agents | 페르소나(`AGENT.md`)와 스킬, 슬래시 커맨드, 서브에이전트 dispatch, 그리고 선택 `greeting` — 빈 대화의 첫 줄로, 창구가 아니라 말하는 쪽 소속이다. 하네스에는 중립 번들로 전달되고, 네이티브 형식으로의 번역은 전부 어댑터 소유. `default: true` 가 착지 에이전트 — 에이전트를 선언한 패키지는 착지가 정해져야 하고(그 표기 또는 짧은 이름과 같은 에이전트), 아니면 설치가 거부한다. 세션은 페르소나 위에 서므로 `agents[]` 밖 이름으로는 턴이 열리지 않는다. 직접 대화는 선언하지 않는다 — 에이전트가 있고 `view` 가 없으면 `/pkg/<이름>/view/` 에 전체 화면 대화가 공짜로 선다. |
| Scripts | 동사. `scripts/<이름>.ts`가 `async (input, ctx) => JSON`을 기본 수출. `scripts.get` 은 `GET` 으로도 답하는 동사를 지명한다 — 인가 리다이렉트, 웹훅 검증 챌린지, 사람이 여는 링크. 질의 문자열이 그대로 입력이고, 문자열을 돌려주면 그것이 본문 전부다. 기본값이 아니라 선언인 이유: 그 문에는 CSRF 판정이 읽을 `Origin` 이 없어서, 저작자가 이름을 적은 자리에서만 열리고 고지서에 들어오는 문으로 선다. |
| Services | 형태는 넷. `source`(자기 몸, 컨테이너 또는 프로세스), `url`(원격 MCP 접점), `api`(원격 REST 베이스), `dir`(기판이 문으로 세우는 폴더). 자격은 밖으로 나가는 둘(`url`·`api`)에만 앉는다. `dir` 은 경로가 아니라 이름으로 닿는다 — 동사는 `ctx.service(<이름>).call("list"|"read"|"write"|"remove", …)` 로 쓰고, 세션은 폴더를 직접 만지지 않는다 — 그 폴더를 감싼 동사로만 닿는다. 선언한 경로는 로컬 기본 바인딩이고, 설치 결재가 갈아 끼우며 조직 기판은 같은 이름을 자기 볼륨 좌표로 푼다. 자격은 입력 칸의 형태(`auth.fields` — 채널 `credential.fields` 와 같은 칸 어휘에, `Authorization` 으로 나갈 토큰 칸 하나에만 `header: true`), 필수 여부(`auth.required`, 미선언 = true; false 면 없어도 돌고 그 기능만 꺼진다), 발급처 안내(`auth.help`)를 선언한다. oauth 자격에도 칸이 있다 — 로그인이 묻지 않는 값(계정 번호·저장소 좌표)의 자리이고, 인가가 만든 번들 안에 함께 앉는다. `auth.accounts: true` 는 한 서비스를 여러 계정으로 쓴다는 선언이다: 금고 좌표가 `<패키지>/<서비스>@<계정>` 이 되고, 동사는 `ctx.service(<이름>).account(<계정>)` 으로 고르고 `.accounts()` 로 열거하며, 고르지 않은 손잡이는 대신 골라 주지 않고 사유와 함께 선다. 콘솔의 연결 화면(`/connect`)이 이 선언으로 전 패키지의 바깥 자격을 그리고, 동사는 비밀을 쥐지 않는다 — `ctx.service(<이름>).connected()` 로 있는지 묻고 `.fields()` 로 비밀 아닌 칸만 읽는다. |
| Connector | 몸 없는 커넥터 — 동사가 외부 REST API 를 부르는 패키지. `api` 서비스가 REST 베이스와 `auth` 형태를 선언하고, 값은 vault 의 `<패키지>/<서비스>` 에 앉는다. 기판이 호출마다 붙이므로 동사는 자격을 손에 쥐지 않고, 선언한 base 밖으로도 나가지 못한다. `auth.scheme` 은 `Authorization` 접두를 정한다(예: Unsplash 의 `Client-ID`) — 미선언 = `Bearer`. `auth.inject` 는 자격을 헤더 밖으로 옮긴다 — 토큰을 파라미터로 받는 API 를 위해 `{query: <이름>}` 또는 `{form: <이름>}`. `bases` 는 한 제공자가 흩어 놓은 다른 호스트(다른 도메인에 사는 토큰 교환)를 선언하고, 그 주소들도 같은 접두 판정을 지나므로 고지서는 자격이 닿는 곳을 전부 이름 부른다. |
...
| Edges | 다른 패키지의 tools, mission, components에 대한 의존 선언. 선언은 신청이고 활성화는 결재 — components는 설치가 edge를 해석할 때 결재가 앉고, 집행점은 기판이 소비자 화면에 심는 import map이다. `agent_access`(tools 형에만, 기본 `scripts-only`)는 소비자의 에이전트가 무엇을 만지는지 정한다 — `scripts-only` 면 edge 도구는 언제나 제공자의 동사이고, `full` 이면 제공자가 `services[].url.tools` 에 선언한 원격 MCP 도구도 raw 로 열리며 고지서가 raw 로 표시한다. |
| Workspace | 패키지의 폴더 결재. 세션의 cwd이며 설치 때 정해져(기본 `~/Relay/<이름>`) 장부에 남는다. 자기 view 는 `GET /pkg/<이름>/workspace/<경로>` 로 읽는다 — 읽기전용, `dir` 문과 같은 감금, 요청마다 재검증. |
...
6. **최소 지반.** 세션이 딛는 땅은 설치 때 결재된 workspace 하나다. 폴더가 더 필요하면 `dir` 서비스를 선언한다 — 그 폴더에는 패키지의 동사만 닿고(`ctx.service`), 세션은 직접 만지지도 경로를 알지도 못한다. 기판 홈(`~/.relay`)은 모든 세션에서 항상 막히고 `dir` 로도 열 수 없다(설치가 거부한다).
...
| `~/.relay/run/daemon.{pid,port,runner}` | 도는 데몬의 pid·포트와 그것이 뜬 러너 — 기동 때 적고, CLI가 따라가고, 종료 때 지운다. 다른 러너로 뜬 기동은 옛 데몬을 물려받는다(앱을 업데이트하면 데몬도 바뀐다) |
