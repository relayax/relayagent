...
| Harness | 运行包内智能体的执行适配器。**适配器是共享的,不属于某个包**:控制台包内置 Claude Code、Codex、Kimi、Pi,基座把它们摊入一个池,所有包都从中取用——未声明 `variants` 即等于全部可选。内置是为了带来池中没有的 harness。要收窄选择,在 `harness.requires` 中写**能力而非 harness 名字**(这样锁的理由留在声明里,别的 harness 实现该能力后会自动重新开放);作者偏好写在 `harness.prefers`,实际验证过的写在 `harness.verified`。凭据坐落于 `llm/<provider>`,跨包共享,在「连接」页面接入。动词:`session`、`setup`、`models`、`commands`、`info`(可选 `login`,以及 `serve` —— 常驻会话,经 stdin 注入回合,而非每回合一个进程)。契约一致性由 `relay harness-check` 判定,完整契约见 [docs/harness-protocol.md](docs/harness-protocol.md)。variant 所驱动的 CLI 在 `requires.binaries` 中声明——带 `manager`+`package` 配方时,缺失则由基座自行安装(装入 `~/.relay/bin/<包>/`,置于 PATH 最前);variant 的 `binary: <name>` 是对该条目的引用,宿主机上损坏的安装会在 setup 失败时被基座副本替换。 |
