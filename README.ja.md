...
| Connector | 身体のないコネクタ——動詞が外部 REST API を呼ぶパッケージ。`api` サービスが REST ベースと `auth` の形を宣言し、値は vault の `<パッケージ>/<サービス>` に置かれる。基板が呼び出しごとに付与するため、動詞が資格情報を手に持つことはなく、宣言した base の外へも出られない。 |
