...
`RELAY_SESSION`, `RELAY_API`, `RELAY_TOKEN`, `RELAY_PRINCIPAL`, `RELAY_HEARTBEAT_S` (heartbeat
interval in seconds — see the `alive` event; absent or `0` means do not beat), and optionally
`RELAY_MODEL` / `RELAY_EFFORT`. `RELAY_*` is the substrate-to-adapter namespace; adapters must not mint their own
