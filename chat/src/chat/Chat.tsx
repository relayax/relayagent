...
function EmptyStarter({ ctx }: { ctx: RelayCtx }) {
  ...
  const title = creating ? "무엇을 만들까요?"
    ...
    : bind.agent ? `${bind.agent} 와의 대화`
    : ctx.instanceId === "system" ? "Relay 와의 대화" : "무엇이든 물어보세요";
  const hint = creating
    ...
    // 홈에는 입력창이 둘(가운데 만들기 상자·이 패널)이라 이 패널이 무엇인지 한 줄 — 설치·설정 이야기는 여기, 만들기는 홈 상자(2026-08-27)
    : ctx.instanceId === "system" ? "설치된 것을 묻거나 설정을 바꿀 때 쓰는 대화예요. 새 에이전트는 홈의 입력창에 적어 만들어요."
    : null;
