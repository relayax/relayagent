...
export interface CtxSeed {
  ...
  services: Record<string, { url: string; fault?: string }>;
...
export type DoorRequest =
  | { kind: "service.call"; name: string; tool: string; args: unknown }
  | { kind: "service.fetch"; name: string; path: string; init: { method?: string; headers: [string, string][]; body?: string | Uint8Array } }
  | { kind: "service.connected"; name: string }
  | { kind: "service.fields"; name: string }
