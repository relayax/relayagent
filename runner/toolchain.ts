 ...
 *
 * 그래서 매니페스트가 **획득 레시피**를 선언하고(harness.variants[].tool) 기판이 자기 prefix 에
...
export function toolPrefix(pkg: string, variant: string): string {
...
export function toolBin(pkg: string, v: HarnessVariant): string | null {
  if (!v.tool) return null;
  return path.join(binDir(toolPrefix(pkg, v.name), v.tool.manager), v.tool.bin);
}

/** 기판 사본이 실재하는가. 실행 가능 여부까지 본다 — 껍데기만 남은 설치를 준비됨으로 세지 않는다. */
export function toolReady(pkg: string, v: HarnessVariant): boolean {
  const bin = toolBin(pkg, v);
 ...
 * 이 축이 통째로 무의미해진다.
 */
export function toolEnv(pkg: string, v: HarnessVariant, base?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const env = { ...(base ?? process.env) } as NodeJS.ProcessEnv;
  if (!v.tool) return env;
  const dir = binDir(toolPrefix(pkg, v.name), v.tool.manager);
 ...
 */
export function ensureTool(pkg: string, v: HarnessVariant): ToolResult {
  if (!v.tool) return { ok: true, out: "", cached: true };
  if (toolReady(pkg, v)) return { ok: true, out: `${v.tool.bin} 준비됨 (기판 사본)`, cached: true };
  const prefix = toolPrefix(pkg, v.name);
  ...
  if (r.error && (r.error as NodeJS.ErrnoException).code === "ENOENT") {
    return { ok: false, cached: false, out: `${v.tool.manager} 이 없습니다 — 이 하네스의 도구를 기판이 설치하려면 ${v.tool.manager} 가 필요합니다` };
  }
  if (r.status !== 0) return { ok: false, cached: false, out: `${v.tool.manager} 설치 실패:\n${tail}` };
  if (!toolReady(pkg, v)) {
    // 설치는 0 으로 끝났는데 실행 파일이 없다 = 선언과 실체의 불일치(bin 이름이 틀렸거나 패키지가 그 이름을 안 깐다)
    return { ok: false, cached: false, out: `설치는 끝났지만 실행 파일이 없습니다: ${toolBin(pkg, v)} — tool.bin 이 이 패키지가 까는 이름과 다른지 확인하세요` };
  }
  return { ok: true, cached: false, out: `${v.tool.bin} 설치됨 (${v.tool.package}${v.tool.version ? "@" + v.tool.version : ""})` };
}

/** 패키지 제거의 동반 조치 — 기판이 깐 도구 사본도 함께 치운다. */
export function removeTools(pkg: string): void {
