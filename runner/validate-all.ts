...
function viewBuildIssues(pkgDir: string): string[] {
  ...
  const issues: string[] = [];
  const stale = staleViewBuild(viewDir, outDir);
  if (stale) {
    issues.push(
 ...
 */
function staleViewBuild(viewDir: string, outDir: string): { rel: string; gap: string } | null {
  let outMs = 0;
  try {
    outMs = fs.statSync(path.join(outDir, "index.html")).mtimeMs;
  } catch {
...
for (const entry of entries) {
  ...
  const viewIssues = viewBuildIssues(pkgDir);
