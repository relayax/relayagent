...
import type { SessionIO, SessionMessage } from "./session.ts";
import type { ClientWireDeps, ClientWireIO, HarnessConfig, SessionRow } from "./client-wire.ts";
...
const { handleClientWire, tapSessionEvent } = await import("./client-wire.ts");
const { setEnvelopeTap } = await import("./session.ts");
