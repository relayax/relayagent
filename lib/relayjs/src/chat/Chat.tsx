...
// ── 로그아웃 크로스뷰 동기화 ─────────────────────────────────────────────────
// 규약·구현은 relayjs auth-sync.ts 단일 소스(뷰 크롬 AuthWatch 와 공유 — 구 쌍둥이 수렴).
  ...
  return (
    <div className={"rc-model" + (loaded ? "" : " rc-model-loading")} ref={boxRef}
      ...
      <button type="button" className="rc-model-btn" onClick={() => setOpen((o) => !o)}
        ...
        <span className="rc-model-lb">Model <span className={"rc-model-val" + (model ? "" : " ghost")}>({shown})</span>{err && <span className="rc-save-err">저장 실패</span>}</span>
      </button>
      {open && (
