...
export function createChat(opts) {
  ...
  // ── 백그라운드 원장과 유휴 폴링 ──────────────────────────────────────────
  // 봉투의 task 이벤트(protocol 3)를 접는다. 턴이 끝나도 미완 백그라운드가 남아 있으면
  // 가볍게 폴링을 이어간다 — 완료가 만든 자발 턴(reply origin:"task")은 기판이 이력에
  // 앉히므로, 그게 보이면 이력을 다시 읽어 화면에 착지시킨다
  const bgTasks = new Set();
  let eventsFrom = 0; // 신형 기판의 events 오프셋 — 턴 폴링과 유휴 폴링이 이어 쓴다
  let bgTimer = null;
  const track = (ev) => {
    if (!ev || ev.event !== "task" || typeof ev.id !== "string") return;
  ...
  function armBgPoll() {
    ...
    bgTimer = setInterval(async () => {
      if (pumping) return stopBgPoll(); // 새 턴이 시작됨 — 턴 폴링이 이어받는다
      if (inFlight) return;
      inFlight = true;
      const pr = await call(P("/session/" + encodeURIComponent(slot) + "/events?from=" + eventsFrom));
      if (pr.data && typeof pr.data.from === "number") {
        eventsFrom = pr.data.from;
        let landed = false;
        for (const e of Array.isArray(pr.data.events) ? pr.data.events : []) {
          track(e);
          emit("progress", e);
          if (e.event === "reply") landed = true;
        }
        if (landed) {
          await loadHistory(); // 자발 턴이 이력에 앉았다 — 화면이 history 이벤트로 다시 그린다
        }
      }
      if (!bgTasks.size) stopBgPoll();
      inFlight = false;
    }, 3000);
  }
  ...
  async function pump() {
    ...
    while (queue.length) {
      ...
      // 신형 기판은 from 오프셋을 돌려줘 새 줄만 나른다 — 구형은 전체 장부라 seen 으로 자른다
      let seen = 0;
      eventsFrom = 0; // 기판이 턴마다 장부를 새로 시작한다
      let polling = false;
      const poll = setInterval(async () => {
        ...
        const pr = await call(P("/session/" + encodeURIComponent(slot) + "/events?from=" + eventsFrom));
        if (pr.data) {
          const evs = Array.isArray(pr.data.events) ? pr.data.events : [];
          if (typeof pr.data.from === "number") {
            eventsFrom = pr.data.from;
            for (const e of evs) {
              track(e);
              emit("progress", e);
      ...
      const r = await call(P("/chat"), {
        ...
        // 화면 맥락 — 합성은 기판 몫이다. 프롬프트 서문으로 붙고 이력에는 원문만 남는다
        ...(item.scene ? { scene: String(item.scene) } : {}),
      });
      clearInterval(poll);
      if (r.error) {
        ...
        if (r.data.usage || r.data.model || r.data.context) {
          emit("usage", { usage: r.data.usage || null, model: r.data.model || null, context: r.data.context || null });
        }
    ...
    setBusy(false);
    pumping = false;
    // 봉투가 알린 백그라운드가 남아 있다 — 완료의 자발 턴을 줍기 위해 가볍게 이어 듣는다
    if (bgTasks.size) armBgPoll();
  }
  ...
  function send(text, opts) {
    ...
    return new Promise((resolve) => {
      queue.push({ text: msg, atts, resolve, agent: (opts && opts.agent) || null, scene: (opts && opts.scene) || null });
      pump();
    });
  }
  ...
  // ask(질문) 회송 — 봉투가 낸 ask 이벤트의 답. answers = [{question, selected[]}]
  async function answer(id, answers) {
    const r = await call(P("/session/" + encodeURIComponent(slot) + "/answer"), { id, answers });
    return r.error ? { error: r.error } : r.data; // { ok }
  }
  ...
  // 서버에 이 슬롯의 턴이 아직 돌고 있으면(새로고침 등으로 busy 를 잃은 화면) 진행 표시와
  // 중지 버튼을 되살리고, 끝날 때까지 가볍게 지켜보다 종결을 이력으로 착지시킨다
  let serverBusyTimer = null;
  function watchServerBusy() {
    if (serverBusyTimer) return;
    serverBusyTimer = setInterval(async () => {
      if (pumping) {
        clearInterval(serverBusyTimer);
        serverBusyTimer = null;
        return;
      }
      const r = await call(P("/session/" + encodeURIComponent(slot) + "/history"));
      if (r.data && !r.data.busy) {
        clearInterval(serverBusyTimer);
        serverBusyTimer = null;
        await loadHistory();
        if (!pumping) setBusy(false);
      }
    }, 3000);
  }
  ...
  const sessions = {
    ...
    async open(s) {
      stopBgPoll(); // 유휴 폴링이 옛 슬롯을 물고 있으면 안 된다
      bgTasks.clear();
      slot = String(s);
      history.length = 0;
      emit("session", slot);
  ...
  return {
    pkg, history, on, send, cancel, answer, reset, meta, ready, harness, sessions, upload, fileUrl,
    get slot() { return slot; },
    get busy() { return busy; },
    get bgCount() { return bgTasks.size; },
  };
}
