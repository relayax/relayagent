...
.pv-step-kids { margin-left: 18px; padding-left: 6px; border-left: 1px dashed var(--rc-line-soft, #eef0f2); }
.pv-step-kids:empty { display: none; }
/* 실황 헤드의 토큰 티커 */
...
export function mount(opts) {
  ...
  // 참조를 보관한다 — remove() 가 해제해야 하는 문서 레벨 리스너 (다른 document 배선과 같은 계약)
  const docClick = (ev) => {
  ...
  function createPane(slot) {
    ...
    function showRunning() {
      ...
      const proc = running.querySelector(".rw-proc");
      appendRows(proc, procRows);
      drawUsage();
      log.appendChild(running);
      log.scrollTop = log.scrollHeight;
    }

    // 행 무리를 중첩 구조로 붙인다 — parent 가 달린 도구 행(서브에이전트의 호출)은
    // 그 위임 행의 kids 로 들어간다. 부모를 못 찾으면 평평하게 강등한다
    function appendRows(container, rows) {
      for (const r of rows) {
        if (r.kind === "tool" && r.parent) continue;
        container.appendChild(renderRow(r));
      }
      for (const r of rows) {
        if (!(r.kind === "tool" && r.parent)) continue;
        const p = rows.find((x) => x.kind === "tool" && x.id === r.parent);
        const host = p && p.el && p.el.querySelector(".pv-step-kids");
        (host || container).appendChild(renderRow(r));
      }
    }
    ...
    function stepMeta(name, detail, argsStr) {
      ...
      try {
        ...
        // 서브에이전트 위임(Task 류) — 대상은 서브에이전트 이름이 정본이다 (system→agent-builder 등)
        if (a.subagent_type) target = a.subagent_type + (a.description ? " — " + a.description : "");
        else target = a.description ?? a.file_path ?? a.path ?? a.pattern ?? a.command ?? a.url ?? a.query ?? a.skill ?? "";
      } catch { /* args 없는 구봉투 */ }
    ...
    function renderRow(row) {
      ...
      const d = document.createElement("div");
      d.className = "pv-step";
      d.innerHTML = '<div class="pv-step-h"><i></i><b></b><span></span><em></em></div><div class="pv-step-b"></div><div class="pv-step-kids"></div>';
      row.el = d;
      paintStep(row);
    ...
    client.on("progress", (ev) => {
      ...
      if (ev.event === "delta" && ev.text) {
        ...
        const row = {
          ...
          parent: typeof ev.parent === "string" ? ev.parent : null,
          args: typeof ev.args === "string" ? ev.args : "", result: "",
          ok: true, done: false, t0: Date.now(), ms: 0,
        };
        procRows.push(row);
        // 서브에이전트의 호출은 위임 행 아래로 중첩 — 부모가 아직 없으면 평평하게
        const pRow = row.parent ? procRows.find((x) => x.kind === "tool" && x.id === row.parent) : null;
        const host = pRow && pRow.el && pRow.el.querySelector(".pv-step-kids");
        (host || proc).appendChild(renderRow(row));
        stick();
