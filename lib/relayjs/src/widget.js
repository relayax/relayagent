...
/* float = 우측 전체 높이 사이드바. 열려 있는 동안 fab 은 숨는다 */
.rw-panel {
...
.rw-inline .rw-panel { position: relative; width: 100%; height: 100%; border-radius: 10px; box-shadow: none; display: flex; }
...
.rw-agent {
  flex: 0 1 auto; max-width: 38%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
  font-size: 11px; font-weight: 600; color: var(--rc-accent-strong, #115e59);
  background: var(--rc-accent-soft, rgba(13, 148, 136, 0.10)); border-radius: 6px; padding: 2px 7px;
}
.rw-sp { flex: 1 1 auto; }
.rw-ctx { flex: 0 0 auto; font-size: 10.5px; font-weight: 600; color: var(--rc-faint, #98a1aa); font-variant-numeric: tabular-nums; border: 1px solid var(--rc-line, #e6e9ec); border-radius: 6px; padding: 1px 6px; }
.rw-head button {
...
.rw-running .rw-head { display: flex; align-items: center; gap: 8px; position: sticky; bottom: 0; z-index: 2; padding: 4px 0; background: var(--rc-bg, #fff); }
...
.rw-panel.setting .rw-set { display: block; }
.rw-panel.setting .rw-log, .rw-panel.setting .rw-sug, .rw-panel.setting .rw-in { display: none; }
...
.rw-sess-menu {
  position: absolute; top: 42px; right: 10px; z-index: 60; min-width: 220px; max-width: 300px;
  background: var(--rc-bg, #fff); border: 1px solid var(--rc-line, #e6e9ec); border-radius: 10px;
  box-shadow: 0 8px 28px rgba(15, 23, 42, 0.14); padding: 4px; max-height: 320px; overflow-y: auto;
}
.rw-sess-item {
...
export function mount(opts) {
  ...
  injectCss();
  const client = createChat(opts);

  const root = document.createElement("div");
  root.className = mode === "inline" ? "rw-inline" : "rw-dock";
  root.innerHTML = `
  <div class="rw-panel${mode === "inline" ? " open" : ""}">
    <div class="rw-head"><span class="rw-d"></span><span class="rw-t">${esc(opts.pkg)}</span><span class="rw-agent" style="display:none"></span><span class="rw-sp"></span>
      <button data-a="sess">대화</button>
      <button data-a="set" style="display:none">설정</button>
      <button data-a="reset">새 대화</button>${mode === "float" ? '<button data-a="close">닫기</button>' : ""}</div>
    ...
    <div class="rw-in">
      ...
      <div class="rw-in-foot">
        ...
        <button class="rw-modelbtn" data-a="set" title="모델 설정">모델 <b class="md-cur">기본</b></button>
  ...
  // 사이드바는 본문을 덮지 않고 밀어낸다: 열릴 때 문서 오른쪽에 패널 폭만큼 여백을 준다.
  // view 의 그리드가 1fr 이라 남은 폭으로 재배치된다 (100vw 고정 레이아웃은 안 밀린다)
  function setOpen(open) {
    ...
    doc.style.paddingRight = open ? panel.getBoundingClientRect().width + "px" : "";
  }

  const log = root.querySelector(".rw-log");
  const set = root.querySelector(".rw-set");
  const sug = root.querySelector(".rw-sug");
  const input = root.querySelector(".rw-in textarea");
  // 입력창 높이는 내용이 정한다. 값을 코드로 바꾼 자리에서도 반드시 다시 재야 한다
  function grow() {
    input.style.height = "auto";
    input.style.height = Math.min(input.scrollHeight, 168) + "px";
  }
  const att = root.querySelector(".rw-att");
  const fileInput = root.querySelector('input[type="file"]');
  const dot = root.querySelector(".rw-d");
  ...
  function draw(m) {
    ...
    el.className = "rw-msg " + (m.role === "user" ? "u" : m.role === "bot" ? "b" : "s");
    if (m.role === "bot") el.innerHTML = mdRender(m.text);
    ...
    if (Array.isArray(m.files) && m.files.length) {
      ...
      for (const f of m.files) {
        ...
        a.href = client.fileUrl(f.path, true);
    ...
    if (m.role === "bot") probeFileLinks(el, m.text);
  ...
  function probeFileLinks(el, text) {
    ...
    Promise.all(
      ...
      if (!ok.length || !el.isConnected) return;
      const row = document.createElement("div");
      row.className = "rw-files";
      for (const p of ok) {
        const a = document.createElement("a");
  ...
  function renderAtt() {
    ...
    att.innerHTML = pendingAtts
      .map((a, i) =>
        `<span class="rw-chipf${a.error ? " err" : ""}"><b>${esc(a.name)}</b><i>${a.error ? esc(a.error) : a.path ? fmtSize(a.size) : a.pct + "%"}</i><button data-rm="${i}" title="제거">x</button></span>`)
      .join("");
    att.querySelectorAll("button[data-rm]").forEach((b) =>
      b.addEventListener("click", () => {
        pendingAtts.splice(Number(b.getAttribute("data-rm")), 1);
        renderAtt();
      }));
  }
  ...
  async function addFiles(files) {
    for (const f of Array.from(files || [])) {
      const entry = { name: f.name || "file", size: f.size, pct: 0 };
  ...
  client.on("busy", (b) => {
    ...
    if (b) showRunning();
      ...
      for (const el of queuedEls) el.classList.remove("q");
      queuedEls = [];
    }
  });
  ...
  client.on("turn", () => {
    const el = queuedEls.shift();
    if (el) el.classList.remove("q");
  });
  ...
  const ring = root.querySelector(".rw-ring");
  ...
  const mdCur = root.querySelector(".md-cur");
  ...
  const effEl = root.querySelector(".rw-eff");
  const effCur = root.querySelector(".ef-cur");
  const effDots = root.querySelector(".rw-eff-dots");
  ...
  client.meta().then((m) => {
    if (!m.chat || m.chat.mode === "none") {
      ...
      else draw({ role: "sys", text: "chat 표면이 없는 패키지입니다" });
      return;
    }
    ...
    const pill = root.querySelector(".rw-agpill");
  ...
  const agMenu = root.querySelector(".rw-ag-menu");
  function renderAgMenu() {
    ...
    agMenu.querySelectorAll("[data-agpick]").forEach((b) => {
      b.addEventListener("click", () => {
        ...
        root.querySelector(".rw-agpill .ag-name").textContent = pillAgent;
        agMenu.hidden = true;
      });
    });
  }
  ...
  async function renderSessMenu() {
    ...
    const rows = r.sessions.map((s) => `
      <div class="rw-sess-item${s.slot === client.slot ? " on" : ""}" data-slot="${esc(s.slot)}">
        <span class="rw-sess-name">${esc(s.label)}</span>
        <span class="rw-sess-time">${rel(s.updated)}</span>
        <span class="rw-sess-acts">
          <button class="rw-sess-act" data-op="rename">이름</button>
          <button class="rw-sess-act rw-sess-danger" data-op="del">삭제</button>
        </span>
      </div>`).join("");
    sess.innerHTML = (rows || '<div class="rw-sess-note">저장된 대화가 없습니다</div>') + '<button class="rw-sess-new">+ 새 대화</button>';
  ...
  function startRename(row, slot) {
    ...
    const done = async (save) => {
      if (closed) return;
      closed = true;
      if (save && inp.value.trim()) await client.sessions.rename(slot, inp.value.trim());
      renderSessMenu();
    };
  ...
  function confirmDelete(btn, slot) {
    if (btn.getAttribute("data-armed")) {
      client.sessions.remove(slot).then(() => renderSessMenu());
      return;
    }
  ...
  async function renderSettings() {
    ...
    const hs = [];
    const assetU = (rel) => "/pkg/" + encodeURIComponent(client.pkg) + "/asset/" + rel;
    ...
    function bindFree(mfreeRow) {
      ...
      mfreeRow.addEventListener("click", () => {
        ...
        const applyFree = async () => {
          ...
          const el = document.createElement("div");
          el.className = "rw-lvrow on";
          el.setAttribute("data-mv", picked);
          el.innerHTML = `${micHtml}<span class="rw-lvtx"><b>${esc(picked)}</b><span>직접 입력됨</span></span><span class="rw-lvring"></span>`;
          mvRows.forEach((x) => x.classList.remove("on"));
          wrap.replaceWith(el);
          bindModelRow(el);
          el.insertAdjacentHTML("afterend", '<div class="rw-lvrow rw-lvadd" data-mfree="1"><b>+</b> 직접 입력</div>');
          bindFree(el.nextElementSibling);
        };
  ...
  root.addEventListener("click", (ev) => {
    ...
    if (!sess.hidden && !sess.contains(t) && a !== "sess") sess.hidden = true;
    ...
    else if (a === "reset") client.sessions.create();
    else if (a === "set") toggleSettings();
    else if (a === "agent") {
