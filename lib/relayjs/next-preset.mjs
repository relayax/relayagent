...
//
// 기판 중립 산물(view 정합 V2a) — 정본은 relayagent-oss lib/relayjs/next-preset.mjs 이고 이
// 사본은 벤더링(deploy/relayagent.lock — 무패치·upstream-first)이다. 두 기판의 basePath 축을
// 다 받는다: OSS 는 설치 빌드가 RELAY_BASE_PATH env 를 주입하고(runner/build.ts), relayos 는
...
// 그대로 합성된다.
//
// 사용:  import relayNext from "@relay/relayjs/next-preset.mjs";
...
// 조직 컴포넌트(@local/*) 자동 transpile — components edge 로 소비되는 패키지는 TS 소스
// tgz 로 오므로(발행 npm pack) view 마다 transpilePackages 를 손으로 적으면 소비가 늘 때마다
// 전 view 의 config 를 고쳐야 한다. package.json 의존에서 @local/* 를 자동 수집한다.
// best-effort: package.json 이 없는 경로에선 빈 목록(깨지지 않음).
function localComponentDeps(appDir) {
  try {
    const pj = JSON.parse(readFileSync(path.join(appDir, "package.json"), "utf8"));
    return Object.keys(pj.dependencies ?? {}).filter((k) => k.startsWith("@local/"));
  } catch {
    return [];
  }
}
...
export default function relayNext(metaUrl, user = {}) {
  ...
  return {
    ...
    transpilePackages: [
      ...new Set(["@relay/relayjs", ...localComponentDeps(appDir), ...transpilePackages]),
    ],
    ...rest,
    // env 는 rest 뒤에 둔다 — 버전 주입이 저작자 env 를 덮지 않되(저작 키 보존) 항상 실린다.
    env: { NEXT_PUBLIC_RELAY_VERSION: readPackageVersion(appDir), ...(rest.env ?? {}) },
    webpack: (config, ctx) => {
      // relayjs 소스는 ESM 관습대로 상대 import 에 .js 확장자를 쓴다(실체는 .ts/.tsx).
